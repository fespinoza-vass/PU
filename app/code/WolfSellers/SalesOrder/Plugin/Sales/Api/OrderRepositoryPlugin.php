...
class OrderRepositoryPlugin
{
    ...
    public function afterGet(
        ...
        try{
            ...
            $billingAddressExtensionAttributes->setIdentificacion( ($order->getData('customer_identificacion')=="868") ? "DNI" : "Pasaporte");
            ...
            if ($shipmentAssigments = $orderExtensionAttributes->getShippingAssignments()) {
                ...
                $shippingExtensionAttributes->setRuc($shippingAddress->getRuc());
                $shippingExtensionAttributes->setReferenciaEnvio($shippingAddress->getReferenciaEnvio());
                $shippingExtensionAttributes->setIdentificacion( ($order->getData('customer_identificacion')=="868") ? "DNI" : "Pasaporte");
                $shippingExtensionAttributes->setRazonSocial($shippingAddress->getRazonSocial()??"");
            }
