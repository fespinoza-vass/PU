define([
    ...
    var shippingMixin = {
        defaults:{
            ...
            exports: {
                "goToResume":"checkout.steps.billing-step.payment:IsDisablePaymentMethods"
            }
        },
        isFormInline: true,
        ...
        initialize: function () {
            ...
            this.isShippingStepFinished.subscribe(function (value) {
                ...
                shippingPayment.setShippingModelData();
                this.setIsDisabledShippingStep();
            },this);
            ...
            this.rates.subscribe(function (value) {
                if(!_.isUndefined(value) && !this.isStorePickUpSelected()){
                    ...
                    if( !_.isUndefined(value) &&  this.findRateByCarrierCode('freeshipping')){
        ...
        setFastShipping: function () {
            if(!this.isFastShippingDisabled()){
                ...
                this.isRegularShipping(false);
                this.isUrbanoShipping(false);
                this.isUrbanoMethodConfigured(true);
        ...
        isShippingMethodAvailable: function (methodType) {
            ...
            if (carrier){
                ...
                if (!!carrier.error_message && carrier.carrier_code.includes('envio_rapido')){
                    this.isFastShippingDisabled(true);
                }else{
                    if (carrier.carrier_code.includes('envio_rapido')){
        ...
        clearInputsUrbano:  function () {
            registry.get("checkout.steps.shipping-step.shippingAddress.urbano.referencia").reset();
            registry.get("checkout.steps.shipping-step.shippingAddress.urbano.departamento").reset();
            registry.get("checkout.steps.shipping-step.shippingAddress.urbano.provincia").reset();
            registry.get("checkout.steps.shipping-step.shippingAddress.urbano.distrito").reset();
            registry.get("checkout.steps.shipping-step.shippingAddress.urbano.direccion.0").reset();
        },

        clearInputsRapido: function () {
            registry.get("checkout.steps.shipping-step.shippingAddress.fast.referencia").reset();
            registry.get("checkout.steps.shipping-step.shippingAddress.fast.distrito").reset();
            registry.get("checkout.steps.shipping-step.shippingAddress.fast.direccion.0").reset();
        }
