...
class Edit extends \WolfSellers\Consecutive\Controller\Adminhtml\Sequential
{
    ...
    public function execute()
    {
        ...
        $this->_coreRegistry->register('wolfsellers_consecutive_sequential', $model);
        
        // 3. Build edit form
        /** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->resultPageFactory->create();
        $this->initPage($resultPage)->addBreadcrumb(
            $id ? __('Edit Sequential') : __('New Sequential'),
            $id ? __('Edit Sequential') : __('New Sequential')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Sequentials'));
        $resultPage->getConfig()->getTitle()->prepend($model->getId() ? __('Edit Sequential %1', $model->getId()) : __('New Sequential'));
        return $resultPage;
    }
}
