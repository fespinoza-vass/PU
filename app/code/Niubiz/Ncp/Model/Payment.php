...
class Payment extends AbstractMethod
{
    ...
    public function __construct(
        ...
        \Niubiz\Ncp\Model\LogData $logData,
        \Magento\Quote\Model\QuoteFactory $quoteFactory,
        \Magento\Framework\App\Filesystem\DirectoryList $dir,
        \Magento\Framework\Encryption\EncryptorInterface $encryptor,
        CartRepositoryInterface $cartRepository,
        \Niubiz\Ncp\Helper\Data $helperConfig,
        StoreManagerInterface $storeManager,
        \Niubiz\Ncp\Model\Library\Ncp $NcpManager,
        array $data = array()
    ) {
    ...
    public function capture(\Magento\Payment\Model\InfoInterface $payment, $amount)
    {
       ...
       if (!$payment->hasAdditionalInformation('token') && trim($this->registry->registry('sessionToken')) == '' ) {
        ...
        $debug = $this->helperConfig->getConfig('payment/Ncp_pay/NcpConfiguracion/debug');
        ...
        if($tokenType == 'confirm_success'){
            ...
            if($currencyCode=="USD")
            {
                $merchant_id=$this->helperConfig->getConfig('payment/Ncp_pay/NcpConfiguracion/merchant_id_dollar');

            }elseif($currencyCode=="PEN"){
                $merchant_id=$this->helperConfig->getConfig('payment/Ncp_pay/NcpConfiguracion/merchant_id');

            }

          /*  $merchant_id = $this->helperConfig->getConfig('payment/Ncp_pay/NcpConfiguracion/merchant_id');*/
            $access_key = $this->encryptor->decrypt($this->helperConfig->getConfig('payment/Ncp_pay/NcpConfiguracion/public_key'));
            $SecretAccessKey = $this->encryptor->decrypt($this->helperConfig->getConfig('payment/Ncp_pay/NcpConfiguracion/private_key'));
            $debug = $this->helperConfig->getConfig('payment/Ncp_pay/NcpConfiguracion/debug');
            ...
            try {
                ...
                $rawRespuestaVisa = $this->NcpManager->authorization($ambiente, $sessionKey, $amount,$transactionToken, $quote_Id,$merchant_id,$currencyCode);
                ...
                if( trim($statusCode) == 200){
                   ...
                   $tarjeta="";
                    ...
                    $id_unico = $resultado['dataMap']['ID_UNICO'];  // ID único de la transacción del sistema Ncp
